...
@Component({
  selector: 'app-insurancecompany-update-form',
  templateUrl: './insurancecompany-update-form.component.html',
  styleUrls: ['./insurancecompany-update-form.component.scss']
})
export class InsurancecompanyUpdateFormComponent extends AbstractComponent implements OnInit {
  ...
  setValues(): void{
    if (this.insurancecompanystatusField.pristine) {
      this.insurancecompanystatusField.setValue(this.insurancecompany.insurancecompanystatus.id);
    }

    if (this.nameField.pristine) {
      this.nameField.setValue(this.insurancecompany.name);
    }

    if (this.contact1Field.pristine) {
      this.contact1Field.setValue(this.insurancecompany.contact1);
    }
    if (this.contact2Field.pristine) {
      this.contact2Field.setValue(this.insurancecompany.contact2);
    }

    if (this.addressField.pristine) {
      this.addressField.setValue(this.insurancecompany.address);
    }

    if (this.emailField.pristine) {
      this.emailField.setValue(this.insurancecompany.email);
    }

    if (this.faxField.pristine) {
      this.faxField.setValue(this.insurancecompany.fax);
    }
  ...
  async submit(): Promise<void> {
    ...
    const newinsurancecompany: Insurancecompany = new Insurancecompany();
    newinsurancecompany.insurancecompanystatus = this.insurancecompanystatusField.value;
    newinsurancecompany.name = this.nameField.value;
    newinsurancecompany.contact1 = this.contact1Field.value;
    newinsurancecompany.contact2 = this.contact2Field.value;
    newinsurancecompany.address = this.addressField.value;
    newinsurancecompany.email = this.emailField.value;
    newinsurancecompany.fax = this.faxField.value;



    try{
      const resourceLink: ResourceLink = await this.insurancecompanyService.update(this.selectedId, newinsurancecompany);
